...
function runFullLoop(opts) {
  ...
  if (!opts.stage || opts.stage === 'card') {
    ...
    for (const name of summary.cards) {
      ...
      // cwd 用测试目录，让 main.test.js 里的 require('../../cube-engine') / require('../../config') 解析正确
  ...
  if (opts.report) {
    ...
    // 同时把"最新一次"的副本写到 out/report.last.json 供 CI / IDE 抓取
