...
```bash
cd /Users/hanamaki_mac_mini/Documents/github/act-cube/skill
...
# 3) 把 <name> 加到 skill/config.yaml 的 cards 段
# 4) 跑测试
node cli.js --skip-build
```
...
- `agent/excel-converter.js`：CSV（自实现引号转义）/ XLSX / XLS（`xlsx` npm 包，落到 `skill/node_modules/xlsx`，cli 在 act-cube/ 跑时走绝对路径 fallback）；按 `card` 列分组输出 `src/<card>/case.md`（单卡 csv 无 card 列时用 `--agent-gen <card>` 自动归属）
