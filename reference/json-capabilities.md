...
把 skill 用到新项目，只需：
1. 修改 `config.yaml`，把 `cards` 数组改成新项目的卡片名；按需调整 `agent` 段
2. 复制 `validate/rules.config.json`，根据新项目规范调整：
   - 不需要的规则 → `"enabled": false`
   - 需要放宽的白名单 → 在对应 `options` 加 `allowedXxx`
   - 想升级严重度 → 改 `severity`
3. 把 skill 放到 `act-cube/`（或任意目录），用 `CARDS_ROOT=/path/to/cards CARDS_SUBDIR=src node cli.js --skip-build` 跑
