 ...
 *   2) config.yaml 的 agent 段（skill 根目录统一登记）
 *
 * 结构校验说明：
 ...
 * 优先级：env > config.yaml（agent 段） > 默认值
 * @returns {{
 *   enabled: boolean,
 *   provider: string,
 *   baseURL: string,
...
function requireAgent() {
  ...
  if (!cfg.enabled) {
    throw new Error('agent 未启用：请把 config.yaml 的 agent.enabled 设为 true 或设 ACT_AGENT_ENABLED=true');
  }
  return cfg;
}
